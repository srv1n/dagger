//! CLI-driven DAG Flow demo that loads YAML workflows and executes them.

use anyhow::{anyhow, Result};
use dagger::coord::{ActionRegistry, NodeCtx};
use dagger::dag_flow::{Cache, DagNodeContextData, Node};
use dagger::{
    action, insert_value, parse_input_from_name, serialize_cache_to_prettyjson, DagConfig,
    DagExecutor,
};
use serde_json::json;
use std::env;
...
fn node_from_ctx(ctx: &NodeCtx) -> Result<&Node> {
    DagNodeContextData::from_ctx(ctx)
        .map(|data| &data.node)
        .ok_or_else(|| anyhow!("missing node metadata in context"))
}

/// Adds two numbers together
#[action(name = "add_numbers")]
async fn add_numbers(ctx: &NodeCtx) -> Result<serde_json::Value> {
    let node = node_from_ctx(ctx)?;
    let num1: f64 = parse_input_from_name(&ctx.cache, "num1".to_string(), &node.inputs)?;
    let num2: f64 = parse_input_from_name(&ctx.cache, "num2".to_string(), &node.inputs)?;
    let sum = num1 + num2;
    ...
    insert_value(&ctx.cache, &node.id, "output_sum", sum)?;
...
/// Squares a number
#[action(name = "square_number")]
async fn square_number(ctx: &NodeCtx) -> Result<serde_json::Value> {
    sleep(Duration::from_millis(100)).await;

    let node = node_from_ctx(ctx)?;
    let input: f64 = parse_input_from_name(&ctx.cache, "input".to_string(), &node.inputs)?;
    let squared = input * input;

    insert_value(&ctx.cache, &node.id, "input_terms", format!("{}", input))?;
    insert_value(&ctx.cache, &node.id, "output_squared", squared)?;

    println!("  [{}] Computing: {}² = {}", node.id, input, squared);

    Ok(json!({ "squared_result": squared }))
}

/// Triples a number and adds a string output
#[action(name = "triple_number_and_add_string")]
async fn triple_number_and_add_string(ctx: &NodeCtx) -> Result<serde_json::Value> {
    let node = node_from_ctx(ctx)?;
    let input: f64 = parse_input_from_name(&ctx.cache, "input".to_string(), &node.inputs)?;
    let tripled = input * 3.0;

    insert_value(&ctx.cache, &node.id, "input_terms", format!("{}", input))?;
    insert_value(&ctx.cache, &node.id, "output_tripled", tripled)?;

    println!("  [{}] Computing: {} × 3 = {}", node.id, input, tripled);

    Ok(json!({
        "tripled_result": tripled,
        "test_string": "example_string"
    }))
}

/// Prints a message (for demonstration)
#[action(name = "print_message")]
async fn print_message(ctx: &NodeCtx) -> Result<serde_json::Value> {
    let node = node_from_ctx(ctx)?;
    let message = if node.inputs.is_empty() {
        "No input provided".to_string()
    } else {
        parse_input_from_name(&ctx.cache, "message".to_string(), &node.inputs)
            .unwrap_or_else(|_| "Default message".to_string())
    };

    insert_value(&ctx.cache, &node.id, "output", message.clone())?;
...
fn print_help() {
    println!("\nUsage: cargo run --example dag_flow_cli -- [OPTIONS]\n");
    println!("Options:");
    ...
    println!("\nExamples:");
    println!("  cargo run --example dag_flow_cli -- --list");
    println!(
        "  cargo run --example dag_flow_cli -- --dag parallel_demo --input num1=10 --input num2=20"
    );
    println!("  cargo run --example dag_flow_cli -- --dag infolder --sequential");
}
...
async fn main() -> Result<()> {
    ...
    let mut actions = registry.list();
    actions.sort();
    println!("Registered actions: {}\n", actions.join(", "));

    println!("Loading DAG definitions...");
    let yaml_files = vec![
        "examples/fixtures/pipeline.yaml",
        "examples/fixtures/pipeline_parallel.yaml",
    ];
    ...
    for (dag_name, description) in dags_to_run {
        ...
        let report = executor
            .execute_static_dag(dag_name, &cache, cancel_rx)
            .await?;
        ...
        let failed_nodes: Vec<_> = report.node_outcomes.iter().filter(|o| !o.success).collect();
