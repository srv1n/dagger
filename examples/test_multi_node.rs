...
async fn main() -> anyhow::Result<()> {
    ...
    registry.register(Arc::new(PassThroughAction {
    ...
    let mut executor = DagExecutor::new(Some(config), registry.clone(), "sqlite::memory:").await?;
    ...
    coordinator
        .run_parallel(&mut executor, &cache, "test_dag", "run_001", rx)
        .await?;
