...
use dagger::dag_flow::{
    insert_value, BufferingEventSink, Cache, DagConfig, DagExecutor, LoggingSupervisor, Node,
    NodeSpec, Plan, SupervisorHook,
};
use dagger::{register_action, NodeAction};
...
async fn test_action(_executor: &mut DagExecutor, node: &Node, cache: &Cache) -> Result<()> {
...
"#;
    ...
    let llm_id = plan.add_next_llm(&cont_id, serde_json::json!({"model": "test"}));
