...
use dagger::{insert_value, Cache, DagConfig, DagExecutor, ExecutionObserver};
use serde_json::json;
use std::sync::Arc;
...
impl ExecutionObserver for TestObserver {
    async fn on_node_started(&self, run_id: &str, dag_name: &str, node_id: &str) {
        println!(
            "Node started: run={}, dag={}, node={}",
            run_id, dag_name, node_id
        );
    }

    async fn on_node_completed(&self, run_id: &str, dag_name: &str, node_id: &str) {
        println!(
            "Node completed: run={}, dag={}, node={}",
            run_id, dag_name, node_id
        );
    }
    ...
    async fn on_node_failed(&self, run_id: &str, dag_name: &str, node_id: &str, error: &str) {
        println!(
            "Node failed: run={}, dag={}, node={}, error={}",
            run_id, dag_name, node_id, error
        );
    }

    async fn on_nodes_added(&self, run_id: &str, dag_name: &str, node_ids: &[String]) {
        println!(
            "Nodes added: run={}, dag={}, nodes={:?}",
            run_id, dag_name, node_ids
        );
    }
}
...
async fn main() -> anyhow::Result<()> {
    ...
    // Get the value back
    let result = executor
        .get_node_output_json("test_dag", "llm_node", "llm_out")
        .await?;
    ...
    let tool_result = executor
        .get_node_output_json("test_dag", "tool_node", "tools/response")
        .await?;
