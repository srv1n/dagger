...
use dagger::{
    coord::{
        types::{NodeOutcome, NodeSpec},
        ActionRegistry, Coordinator, EventHook, ExecutionEvent, ExecutorCommand, HookContext,
        NodeAction, NodeCtx, NodeOutput,
    },
    dag_flow::{HumanTimeoutAction, OnFailure, RetryStrategy},
    get_input, insert_value, Cache, DagConfig, DagExecutor,
};
...
impl NodeAction for AddAction {
    ...
    async fn execute(&self, ctx: &NodeCtx) -> anyhow::Result<NodeOutput> {
        let a = ctx.inputs.get("a").and_then(|v| v.as_f64()).unwrap_or(0.0);
        let b = ctx.inputs.get("b").and_then(|v| v.as_f64()).unwrap_or(0.0);
...
async fn main() -> anyhow::Result<()> {
    ...
    // Add initial nodes
    executor
        .add_node(dag_name, "add_1".to_string(), "add".to_string(), vec![])
        .await?;
    executor
        .add_node(
            dag_name,
    ...
    coordinator
        .run_parallel(&mut executor, &cache, dag_name, "run_001", cancel_rx)
        .await?;
