...
impl Recovery {
    ...
    async fn recover_task(&self, task: Task) -> Result<RecoveryOutcome> {
        ...
        match durability {
            DurabilityMode::BestEffort => {
                self.recover_best_effort(task).await
            }
            DurabilityMode::AtMostOnce => {
                self.recover_at_most_once(task).await
            }
        }
    }
    ...
    async fn recover_best_effort(&self, task: Task) -> Result<RecoveryOutcome> {
        ...
        debug!("Recovering task {} with BestEffort mode - will retry", task_id);
    ...
    async fn recover_at_most_once(&self, task: Task) -> Result<RecoveryOutcome> {
        ...
        debug!("Recovering task {} with AtMostOnce mode - will pause", task_id);

        // Update task status to Blocked with recovery reason
        self.storage.update_status(&task_id, TaskStatus::Blocked).await?;
