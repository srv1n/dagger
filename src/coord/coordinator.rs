    ...
    match cmd {
        AddNode { dag_name: dn, spec } => {
            ...
            let id = spec
                .id
                .clone()
                .unwrap_or_else(|| exec.gen_node_id(&spec.action));
            exec.add_node(dag_name, id.clone(), spec.action, spec.deps)
                .await?;
            // Store inputs if provided
            if !spec.inputs.is_null() {
                exec.set_node_inputs_json(dag_name, &id, spec.inputs, cache)
                    .await?;
            }
        ...
        SetNodeInputs {
            ...
            exec.set_node_inputs_json(dag_name, &node_id, inputs, cache)
                .await?;
        }
...
async fn spawn_worker(
    ...
    let ctx = NodeCtx::new(dag_name, node_id.clone(), resolved_inputs, cache.clone());
