...
impl SqliteStorage {
    ...
    pub async fn store_artifact(
        ...
        if data.len() <= self.artifact_config.inline_threshold {
            ...
            self.store_artifact_inline(&artifact_id, artifact, &data_value)
                .await?;
        } else {
            // Store as file
            let file_path = self.generate_artifact_file_path(&artifact_id, &artifact.artifact_type);
            self.store_artifact_file(&artifact_id, artifact, &file_path, data)
                .await?;
        }
    ...
    async fn store_artifact_inline(
        &self,
        artifact_id: &str,
        artifact: &Artifact,
        data: &Value,
            ...
            Some(row) => {
                ...
                if let Some(data_json) = data_opt {
                    ...
                    let mut file = File::open(&file_path).await.map_err(|e| {
                        StorageError::FileOperation(format!(
                    ...
                    Err(StorageError::ArtifactNotFound(format!(
    ...
    pub async fn get_shared_state(&self, key: &str) -> Result<Option<Value>, StorageError> {
        ...
        sqlx::query(
            "DELETE FROM shared_state WHERE ttl IS NOT NULL AND ttl < datetime('now', 'utc')",
        )
        .execute(&self.pool)
        .await?;
...
mod tests {
    ...
    async fn test_shared_state() {
        ...
        // Set value
        storage
            .set_shared_state("test_key", &test_value, None)
            .await
            .unwrap();
