use super::dag_flow::{
    create_execution_report, execute_node_with_context,
    Cache, DagExecutor, DagExecutionReport, ExecutionContext, Node, NodeExecutionOutcome,
    OnFailure, parse_input_from_name,
};
...
pub async fn execute_dag_parallel(
    ...
    loop {
        ...
        if *executor.stopped.read().unwrap() {
        ...
        {
            parse_input_from_name(cache, "iteration".to_string(), &current_dag[supervisor_idx].inputs)
                .unwrap_or(0)
        } else {
            executed_nodes.len()
        };
        ...
        let ready_nodes: Vec<Node> = current_dag
            ...
            .filter_map(|idx| {
                ...
                if !executed_nodes.contains(&node.id) &&
                   node.dependencies.iter().all(|dep| executed_nodes.contains(dep)) {
                    Some(node.clone())
                } else {
                    None
                }
        ...
        for node in ready_nodes {
            ...
            active_tasks += 1;
            info!("Launching node {} (active tasks: {})", node.id, active_tasks);
        ...
        if active_tasks > 0 {
            match rx.recv().await {
                Some(outcome) => {
                    active_tasks -= 1;
                    info!("Node {} completed (active tasks: {})", outcome.node_id, active_tasks);
